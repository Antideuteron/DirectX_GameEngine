...
public:
  static void Init(void) noexcept {}
	static void KeyUp(const uint32_t key) noexcept;
	static void KeyDown(const uint32_t key) noexcept;

	static bool IsPressed(const uint32_t key) noexcept;
	static bool IsReleased(const uint32_t key) noexcept;
